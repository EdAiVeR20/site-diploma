...
const CollapsibleToast = ({
  ...
  return (
    ...
    >
      <div
        className={`flex-shrink-0 flex items-center justify-center ${minimized ? "w-12 h-12" : "w-8 h-8 pl-1.5"} ${pulse && minimized ? "animate-pulse" : ""} transition-all duration-500`}
      >
        {icon}
      </div>

      <div
        className={`transition-all duration-400 overflow-hidden flex items-center justify-between whitespace-nowrap ${minimized ? "w-0 opacity-0" : "opacity-100 flex-1"}`}
      >
        <div className="flex flex-col overflow-hidden">
          <span className="text-sm font-medium pr-1 truncate">{title}</span>
          {description && (
            <span className="text-[11px] text-[var(--tg-theme-hint-color)] truncate leading-tight mt-0.5">
              {description}
            </span>
          )}
        </div>
...
const ExpandedCarDetails = memo(function ExpandedCarDetails({
  ...
  const { data: details, isLoading: detailsLoading } = useCarDetails(car.id);
  ...
  return (
    <div className="flex flex-col min-h-full px-5 pb-28 pt-14 bg-[var(--tg-theme-bg-color)]">
      ...
      <div className="relative h-48 flex items-center justify-center px-6 my-4">
        {car.imageUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <svg
              className="w-24 h-24 text-[var(--tg-theme-hint-color)]/20"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={0.8}
        ...
        {detailsLoading ? (
          <div className="grid grid-cols-2 gap-2">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="h-14 rounded-xl bg-[var(--tg-theme-secondary-bg-color)] animate-pulse"
              />
            ))}
          </div>
        ) : details ? (
          <div className="grid grid-cols-2 gap-2">
            {[
              {
                label: "Год выпуска",
                value: details.year ? String(details.year) : "—",
              },
              { label: "Цвет", value: details.color || "—" },
              { label: "КПП", value: details.transmission || "—" },
              { label: "Привод", value: details.driveType || "—" },
              {
                label: "Мощность",
                value: details.enginePower
                  ? `${details.enginePower} л.с.`
                  : "—",
              },
              { label: "Топливо", value: details.fuelType || "—" },
              {
                label: "Дата последнего ТО",
                value: details.lastServiceDate
                  ? formatDateRu(details.lastServiceDate)
                  : "ТО не проводилось",
              },
              {
                label: "Пробег",
                value: details.mileage
                  ? `${details.mileage.toLocaleString()} км`
                  : "—",
              },
            ].map((spec) => (
              <div
                key={spec.label}
                className="bg-[var(--tg-theme-secondary-bg-color)] rounded-xl px-3.5 py-2.5"
              >
                <p className="text-[11px] text-[var(--tg-theme-hint-color)] mb-0.5">
                  {spec.label}
                </p>
      ...
      {car.tariffs.length > 0 ? (
            ...
            .map((tariff) => {
              ...
              return (
                <button
                  ...
                  className={`flex-shrink-0 w-36 rounded-2xl overflow-hidden transition-all text-left active:scale-95 ${
                    isSelected
        ...
        <div className="text-center py-6 px-4 bg-[var(--tg-theme-secondary-bg-color)] rounded-2xl">
          <p className="text-sm text-[var(--tg-theme-hint-color)]">
            Тарифы ещё не установлены
          </p>
          <p className="text-xs text-[var(--tg-theme-hint-color)] mt-1">
            Аренда временно недоступна
          </p>
        </div>
      )}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-[var(--tg-theme-bg-color)] border-t border-[var(--color-accent)]/10 safe-area-bottom z-[60]">
        <Button
          ...
          onClick={onRent}
          loading={isCreating}
          disabled={!selectedTariff || car.tariffs.length === 0}
        >
          Арендовать
        </Button>
      </div>
...
export const HomePage = memo(function HomePage({
  ...
  // Balance now read from profile query (useProfile), not Redux
  const {
    latitude,
    longitude,
  ...
  // BUG-3: Get hasActiveRental from profile query
  const { data: profile } = useProfile();
  const hasActiveRental = profile?.hasActiveRental ?? false;
  ...
  useEffect(() => {
    ...
    if (toastProps) {
      toast.custom(
        (t) => (
          <div
            className={`${t.visible ? "animate-enter" : "animate-leave"} pointer-events-none mt-1 mr-1`}
          >
            <CollapsibleToast
  ...
  // Map marker tap
  const handleMapCarOpen = useCallback(
    (car: Car) => {
      setExpandedCar(car);
      setIsDrawerOpen(true);
      setSelectedCarId(car.id);
  ...
  const handleRent = useCallback(async () => {
    ...
    if (currentBalance <= 0) {
      ...
      await showAlert(
        "Недостаточно средств на балансе. Пополните баланс перед арендой.",
      );
      return;
    }

    // BUG-3: Check no active rental
    if (hasActiveRental) {
      hapticFeedback("error");
      await showAlert(
        "У вас уже есть активная аренда. Завершите текущую поездку.",
      );
      return;
    }
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      {/* Active Rental Panel — shown when user has an active rental */}
      <ActiveRentalPanel isVisible={isActive && hasActiveRental} />
